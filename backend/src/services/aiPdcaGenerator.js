...
// Neither provider is given a chance to hang the request indefinitely — if
// nothing comes back in this window, we abort and fall through to the next
// provider (or the blank-template fallback) instead of leaving the mentor
// staring at "Generating…" forever.
const AI_TIMEOUT_MS = Number(process.env.PDCA_AI_TIMEOUT_MS) || 20000;
...
// Wraps fetch with a hard deadline. AbortController cancels the request
// (and the socket) rather than merely giving up on waiting for it, so a
// truly stuck connection doesn't stay open in the background either.
 ...
 * a usable blank form rather than crashing the flow. A hung/slow provider
 * (see AI_TIMEOUT_MS above) is treated the same as a failed one — it's
 * aborted and the next provider (or the blank template) takes over.
 */
export async function generatePDCADraft(groundingData) {
  ...
  if (isUsableKey(groqKey)) {
    try {
      aiLog("groq_start", { fellow: groundingData.fellowName, model: GROQ_MODEL, timeoutMs: AI_TIMEOUT_MS });
      draft = await callGroq({ userMessage, apiKey: groqKey });
      provider = "groq";
      aiLog("groq_success", { fellow: groundingData.fellowName });
    } catch (err) {
  ...
  if (!draft && isUsableKey(geminiKey)) {
    try {
      aiLog("gemini_start", { fellow: groundingData.fellowName, model: GEMINI_MODEL, timeoutMs: AI_TIMEOUT_MS });
      draft = await callGemini({ userMessage, apiKey: geminiKey });
      provider = "gemini";
      aiLog("gemini_success", { fellow: groundingData.fellowName });
    } catch (err) {
