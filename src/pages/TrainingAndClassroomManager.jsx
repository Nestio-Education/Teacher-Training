  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        // Field Visit / Anganwadi children data
        externalBeneficiaries: isFieldVisit ? externalBeneficiaries : [],
        visitMetrics: isFieldVisit
          ? {
              ...visitMetrics,
              totalChildrenCount:
                Number(visitMetrics.totalChildrenCount) ||
                externalBeneficiaries.length
            }
          : undefined
      };
      ...
      if (itemType === "lesson") {
        res = await submitLessonCompletion(
          activity._id || activity.id,
          payload
        );
      } else {
        res = await submitActivityCompletion(
          activity._id || activity.id,
          payload
        );
      }
